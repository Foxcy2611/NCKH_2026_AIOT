...
bool SGP30_Init(){
    ...
    SGP30_Write_CMD(SGP30_CMD_GET_FEATURE_SET);
    delay(2); // vTask
    ...
    // Bỏ qua 3 byte data của check version
    Wire.read();
    Wire.read();
    Wire.read();

    // 2. Khởi tạo thuật toán đo đạc Air Quality
    SGP30_Write_CMD(SGP30_CMD_INIT_AIR_QUALITY);
    delay(10); // vtask
...
bool SGP30_Measure(SGP30_Data_t* data){
    SGP30_Write_CMD(SGP30_CMD_MEASURE_AIR_QUALITY);
    delay(12); // vtask
