...
with tempfile.TemporaryDirectory() as tmp:
 for mode in [0,2]:
  ...
  assert all(x['schema_version']==1 and x['message_type']=='complete_packet' for x in rows)
  ...
  assert rows[1]['patient_event'] is not None and rows[1]['event_id']==rows[1]['alert']['event_id']
  assert rows[2]['patient_event'] is None and rows[2]['event_id'] is None
  assert rows[1]['record_id']!=rows[2]['record_id']
  assert rows[1]['patient_age_ms']==1000 and rows[2]['patient_age_ms'] is None
  assert rows[2]['status']['node_valid']
  ...
  print(f'PASS complete packet mode={mode}: missing/clean/dirty/age/event ID/alert threshold/nulls/buffer bounds')
  if mode==0:
