...
void TaskEspNow(void *parameter) {
    ...
    for (;;) {
        ...
        if (!GatewayState_UpdateNode(event)) {
            gatewayStats.state_update_failed++;
            sendResponse(raw, packet, payload.session_id, RESPONSE_NACK_BUSY);
            Serial.println("[RX] State mutex timeout -> NACK_BUSY");
            continue;
        }
        ...
        Serial.printf("[RX] ACCEPT seq=%lu session=%lu -> current_node dirty=1\n",
                      static_cast<unsigned long>(packet.sequence),
