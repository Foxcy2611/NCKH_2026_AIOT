...
**Trách nhiệm:** thu audio I2S, VAD, rolling pre-trigger buffer PSRAM, Audio Quality Check, DSP, TinyML inference, voting, MAX30102 theo phiên, OLED interaction, low-power state, tạo payload sự kiện, mã hóa/xác thực AES-128-GCM, ESP-NOW event TX và xử lý phản hồi bảo mật từ Gateway. Khi Gateway không khả dụng, Patient Node tiếp tục hoạt động standalone và không lưu backlog để đồng bộ về sau.
...
**Trách nhiệm:** ESP-NOW RX; xác thực và giải mã AES-128-GCM; chống nhận trùng; tạo ACK/NACK đã mã hóa; đọc DHT22/BMP280/SGP30 định kỳ; Environment Snapshot; duy trì `current_node` và `current_gate` là hai snapshot mới nhất; tạo `Complete_Packet_t` khi cần publish; quản lý `current_node_valid`, `current_gate_valid` và `node_dirty`; Wi-Fi; LTE qua A7680C; MQTT publish; GPS khi cần; optional TFT/status UI.
...
MONITOR -> INMP441+I2S ON -> 1s rolling PSRAM -> VAD continuous
   ...
   -> TX inflight -> ESP-NOW -> Return to MONITOR
```
...
- Gateway tạm không khả dụng → Patient Node vẫn hoàn thành đo/suy luận/hiển thị cục bộ; lớp truyền thông chỉ retry ngắn hạn rồi bỏ bản truyền của Event, không đồng bộ Event cũ về sau.
...
|---|---|---|
| Packet mới, tag hợp lệ | Giải mã, cập nhật `current_node` đúng một lần và gửi `ACK_ACCEPTED` đã mã hóa | Xác thực ACK và kết thúc `tx_inflight` |
| Packet hợp lệ nhưng trùng `device_id + sequence` | Không xử lý Event lần hai; gửi `ACK_DUPLICATE` đã mã hóa | Coi là đã nhận thành công và kết thúc `tx_inflight` |
...
| Hết số lần retry | Không liên quan | Bỏ `tx_inflight`; Event vẫn đã hoàn thành cục bộ trên Node và không được lưu để đồng bộ lại |
...
Nếu người dùng bỏ qua HR/SpO₂ → `vitals_valid = false`; đây vẫn là một phiên hợp lệ. `device_id`, `sequence`, trạng thái gửi và số lần retry thuộc lớp truyền thông, không thuộc Session. Node không duy trì lịch sử Event để đồng bộ lại sau khi mất Gateway.
...
| 12 byte          | 12 byte  | 16 byte          | 16 byte            |
+------------------+----------+------------------+--------------------+
                         Tổng: 56 byte
```
...
- `ACK_ACCEPTED` hoặc `ACK_DUPLICATE` hợp lệ → kết thúc `tx_inflight`.
- Timeout hoặc phản hồi không hợp lệ → retry đúng packet đang inflight theo giới hạn.
- Hết retry → bỏ bản truyền; không lưu Event để gửi lại khi Gateway xuất hiện về sau.
- Gateway lưu dấu `device_id + sequence` đủ để ACK packet trùng nhưng không xử lý Event lần hai.

### 11.2. TX inflight ngắn hạn

Patient Node chỉ giữ **một secure packet đang chờ ACK** trong RAM để phục vụ timeout/retry. Đây không phải hàng đợi lịch sử và không phải persistent storage.

```text
Event mới -> mã hóa một lần -> tx_inflight -> gửi -> chờ ACK
                                         |-> timeout: retry nguyên packet cũ
                                         |-> ACK hợp lệ: clear tx_inflight
                                         |-> hết retry: clear tx_inflight, không sync lại Event cũ
```

Trong một lần retry không dựng lại payload, không tạo nonce mới và không tăng sequence.
...
```text
Verified/Decrypted Patient Event -> current_node (valid=true, dirty=true)
Gateway sensors/network/GPS        -> current_gate (valid=true)
current_node + current_gate        -> Build Complete_Packet_t -> MQTT Publish
MQTT publish Patient Event success -> node_dirty=false
```

Gateway duy trì latest-state. `Complete_Packet_t` chỉ được dựng khi cần publish; nếu Internet mất, `current_node/current_gate` vẫn tiếp tục phục vụ TFT và Event mới có thể overwrite Event cũ chưa publish.
...
Người dùng mang Patient Node ra khỏi vùng Gateway. Patient Node vẫn hoạt động độc lập: Manual Check, TinyML, HR/SpO2 và OLED Result. Nếu ESP-NOW không nhận được phản hồi sau số lần retry cho phép, Node bỏ bản truyền của Event đó. Không lưu backlog và không đồng bộ Event cũ khi quay lại gần Gateway.
...
**Task proposal:**
```text
TaskEspNow RX -> RawSecurePacketQueue -> Verify/Decrypt -> current_node
                                                    |
                                                    +-> Encrypted ACK/NACK TX
TaskSensor / TaskGps / Network --------------------> current_gate
current_node + current_gate -> TaskGatewayManager -> Complete_Packet_t -> TaskNetwork/MQTT
current_node/current_gate -------------------------> Optional TFT Display
```
...
**Patient Node:** Deep sleep vs light sleep, Battery capacity, MAX30102 measurement duration, OLED timeout.

**ESP-NOW/AES-GCM:** schema packet v1 dùng payload 16 byte và secure packet 56 byte; còn TBD: Retry count, thời gian timeout/backoff, Channel management và cách nạp/thay khóa.
...
PHASE 3 — ESP-NOW EVENT LINK
    Payload 16 byte, AES-128-GCM packet 56 byte, directional keys,
    sequence, secure ACK/NACK, chống trùng, một TX inflight + local retry ngắn hạn

PHASE 4 — GATEWAY RTOS
    SensorTask, EspNowTask, GatewayManager, Network/MQTT, current_node/current_gate state
