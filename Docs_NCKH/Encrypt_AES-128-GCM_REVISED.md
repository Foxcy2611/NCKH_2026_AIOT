...
- Kích thước cố định: `16 byte`.
- Là kết quả mã hóa một payload logic 16 byte.
...
`ACK_DUPLICATE` có nghĩa Gateway đã nhận Event này trước đó. Gateway không xử lý Event lần hai nhưng vẫn xác nhận thành công để Node clear `tx_inflight`.
...
2. Sao chép MAC nguồn và đúng 56 byte packet vào hàng đợi RTOS.
...
|---|---|
| Tag hợp lệ, Event mới và đã cập nhật `current_node` | `RESPONSE_ACK_ACCEPTED` |
| Tag hợp lệ, Event trùng đã xử lý trước đó | `RESPONSE_ACK_DUPLICATE` |
| Tag hợp lệ nhưng Gateway tạm chưa thể nhận/xử lý Event | `RESPONSE_NACK_BUSY` |
...
2. Kích thước phải đúng 56 byte
...
ACK/NACK không mang timestamp; Node không đồng bộ clock từ Gateway. Gateway luôn lặp lại `session_id` của Event đang phản hồi.
...
ACK_ACCEPTED hoặc ACK_DUPLICATE
    -> clear tx_inflight
    -> kết thúc trách nhiệm truyền của Event hiện tại

NACK_BUSY
    -> chờ rồi gửi lại đúng packet mã hóa cũ

NACK_UNSUPPORTED hoặc NACK_INTERNAL
    -> clear tx_inflight
    -> Node tiếp tục hoạt động cục bộ

Không có phản hồi hợp lệ trước timeout
    -> tăng retry_count
    -> gửi lại packet mã hóa cũ
    -> hết retry thì clear tx_inflight; không lưu Event để đồng bộ lại
```
...
## 11. TX inflight và quy tắc retry

Patient Node không duy trì Pending Queue dài hạn. Mỗi thời điểm chỉ cần giữ một secure packet đang chờ ACK:
...
- Không thay đổi một byte nào trong packet đang inflight.
- Gateway không xử lý Event trùng lần hai nhưng vẫn trả `ACK_DUPLICATE`.
- Hết số lần retry: clear `tx_inflight`; Node không lưu backlog và không gửi lại Event cũ khi Gateway quay lại.
...
SESSION_READY
    |
    v
Tạo Payload -> Mã hóa -> tx_inflight
    |
    v
esp_now_send()
    |
    +-- SEND_FAIL / timeout --> retry nguyên packet cũ (tối đa giới hạn)
    |
    +-- ACK hợp lệ ---------> clear tx_inflight
    |
    +-- hết retry ----------> clear tx_inflight, standalone tiếp tục
```
...
3. Gateway tắt: Node retry đủ số lần rồi clear `tx_inflight`; kết quả đo vẫn chỉ tồn tại cục bộ/hiển thị tại Node.
4. Gateway tạm bận: trả `NACK_BUSY`, Node chờ rồi retry.
...
8. Sau khi hết retry hoặc Node reset, Event cũ không được tự động đồng bộ lại.
...
- Event trùng không cập nhật `current_node` lần hai.
- ACK giả hoặc ACK cũ không làm Node clear `tx_inflight`.
...
- Retry gửi lại nguyên secure packet cũ trong thời gian inflight.
- Hết retry thì Node bỏ bản truyền, không lưu backlog.
